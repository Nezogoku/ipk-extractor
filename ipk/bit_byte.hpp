...
static void setBeInt(uchar *dst0, const unsigned int src1, int length) {
    for (int c = 0; c < length; ++c) dst0[(length - 1) - c] = src1 >> (8 * c);
}

///Set variable little-endian int to array
static void setLeInt(uchar *dst0, const unsigned int src1, int length) {
    unsigned int src2 = setReverse(src1, length);
    setBeInt(dst0, src2, length);
}
